        ...
        <div className={`w-full max-w-[${max_width}] border bg-white border-gray-300 rounded-xl mx-auto mt-10 overflow-hidden pb-5 shadow-sm relative group ${hover_out ? 'hover:-translate-y-2 hover:shadow-white hover:shadow-2xl' : ''} transition-all duration-300 ease-linear`}>
            ...
            <div className="flex flex-col px-3">
                ...
                <div className={`bg-orange-100 text-mainCL flex flex-col rounded-xl py-3 mt-2 border-mainCL border text-${fs_text}`}>
                    ...
                    <span className={`mx-auto mt-2 text-${fs_desc}`}>Save {p_sale.variant_ph_org_price - p_sale.variant_ph_final_price} ₫
                        ({Math.round(100 - (p_sale.variant_ph_final_price / p_sale.variant_ph_org_price) * 100)}% off)</span>
                </div>
                <button className={`bg-mainCL mt-5 text-white text-center py-2 rounded-xl text-${fs_title} font-medium`}><a href="">View Details</a></button>
            </div>
